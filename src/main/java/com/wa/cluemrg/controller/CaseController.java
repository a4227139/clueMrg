...
import com.wa.cluemrg.util.NumUtil;
...
public class CaseController {
    ...
    private Workbook generateExcelWorkbook(String dateStart,String dateEnd) throws ParseException {
        ...
        try {
            ...
            //生成2024年全市电诈案件情况统计表的sheet
            Sheet sheetStat = workbook.getSheetAt(2);
            row = sheetStat.getRow(0);
            cell = row.getCell(0);
            // 修改A1，也就是标题行
            cell.setCellValue(caseExcelFileName.replace(".xlsx",""));
            //填充指标
    ...
    //循环案件列表，累加各分县局各级别的数据，例如 城中-1:51 ，不含总计
    ...
    public List<CaseIndex> dealCaseIndex(List<Case> caseList,List<Case> solveCaseList,List<Case> historyCaseList,String dateStart,String dateEnd) throws ParseException {
        ...
        for (Case caseObj:caseList){
            String jurisdiction = caseObj.getJurisdiction();
            CaseIndex caseIndex = caseIndexMap.get(JurisdictionUtil.jurisdictionSeqMap.get(jurisdiction));
            //立案数
        ...
        for (CaseIndex caseIndex :caseIndexMap.values()){
            ...
            //万人发案数
            float countPerW = (float)caseIndex.getCount()/JurisdictionUtil.jurisdictionPopulationMap.get(jurisdiction);
    ...
    public List<Case> syncCaseList(String dateStart,String dateEnd,String urlString) {
        try {
            String html = sendToJZTJ(dateStart,dateEnd,"4502",urlString);
            List<Case> list = parseHtmlToCases(html);
    ...
    public List<CaseStatIndex> syncCaseStatList(String dateStart,String dateEnd,String urlString) {
        try {
            String html = sendToJZTJ(dateStart,dateEnd,"4502",urlString);
            List<CaseStatIndex> list = parseHtmlToCaseStatIndex(html);
            ...
            if (reportTable != null) {
                ...
                for (Element row : rows) {
                    ...
                    if (tdElements.get(0).text().contains("局")||tdElements.get(0).text().contains("市")||tdElements.get(0).text().contains("合计")){
                        ...
                        caseStatIndex.setCount(NumUtil.convertStringToInt(tdElements.get(1).text()));
                        caseStatIndex.setCountHistory(NumUtil.convertStringToInt(tdElements.get(2).text()));
                        caseStatIndex.setSolveCount(NumUtil.convertStringToInt(tdElements.get(24).text()));
                        caseStatIndex.setSolveRateRatio(NumUtil.convertStringToFloat(tdElements.get(26).text()));
                        caseStatIndex.setLossMoney(NumUtil.convertStringToFloat(tdElements.get(29).text()));
                        ...
                        for (int i=1;i<=4;i++){
                            ...
                            if (i==1){
                                caseStatSubIndex.setCount(NumUtil.convertStringToInt(tdElements.get(30).text())+NumUtil.convertStringToInt(tdElements.get(31).text()));
                            }else if (i==4){
                                caseStatSubIndex.setCount(NumUtil.convertStringToInt(tdElements.get(34).text())+NumUtil.convertStringToInt(tdElements.get(35).text()));
                            }else {
                                caseStatSubIndex.setCount(NumUtil.convertStringToInt(tdElements.get(i+30).text()));
                            }
    ...
    //给警综统计模块发请求
    public String sendToJZTJ(String dateStart,String dateEnd,String LADW, String urlString){
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        try {
            if (StringUtils.isEmpty(dateStart)){
                dateStart="2024-01-01";
            ...
            log.error("发送请求错误，url："+urlString,e);
        }
        return "";
    }
